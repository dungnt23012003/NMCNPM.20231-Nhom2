...
public class HoatDongCellRenderer implements ListCellRenderer<GalleryItem> {
    @Override
    public Component getListCellRendererComponent(JList<? extends GalleryItem> list, GalleryItem value, int index, boolean isSelected, boolean cellHasFocus) {
        if (value instanceof HoatDongAdapter castedValue) {
            ...
            JLabel startDateLabel = ComponentFactory.createLabel(castedValue.getHoatDong().ngayBatDau);
            startDateLabel.setForeground(Color.GRAY);
            startDateLabel.setFont(GUIConfig.DefaultFont.deriveFont(12f));
            component.add(startDateLabel);
